...
function removePlacement(document: PanelLayoutDocument, panelId: string): void {
  for (const column of document.columns) column.panelIds = column.panelIds.filter((id) => id !== panelId);
...
function visibleTerminalReplacement(document: PanelLayoutDocument): string | null {
  const candidates = [document.focusedPanelId, ...[...document.floatingPanelIds].reverse(), ...document.columns.flatMap((column) => column.panelIds)]
    .filter((id): id is string => Boolean(id));
  return candidates.find((id) => document.instances[id]?.kind === 'terminal' && !document.instances[id]?.minimized) ??
    Object.values(document.instances).find((panel) => panel.kind === 'terminal')?.id ?? null;
}
  ...
  if (target.floatingRect) {
    ...
    const column =
      ...
      [...document.columns]
        .reverse()
        .sort((left, right) => {
          const expanded = (candidate: typeof left) => candidate.panelIds.filter((panelId) => !document.instances[panelId]?.minimized).length;
          return expanded(left) - expanded(right);
        })[0] ??
      document.columns[0]!;
    const index = Math.min(column.panelIds.length, Math.max(0, target.index ?? column.panelIds.length));
...
const panelLayoutSlice = createSlice({
  ...
  reducers: {
    hydratePanelLayout(_state, action: PayloadAction<unknown>) { return normalizePanelLayoutState(action.payload); },
    createPanel(state, action: PayloadAction<{ kind: PanelKind; target?: PanelCreateTarget }>) {
      if (addInstance(state.activeLayout, action.payload.kind, action.payload.target)) markDirty(state);
    },
    duplicatePanel(state, action: PayloadAction<{ sourcePanelId: string; target?: PanelCreateTarget }>) {
    ...
    closePanel(state, action: PayloadAction<PanelInstanceId>) {
      ...
      if (state.activeLayout.terminalOwnerPanelId === id) state.activeLayout.terminalOwnerPanelId = visibleTerminalReplacement(state.activeLayout);
      markDirty(state);
    },
    togglePanelMinimized(state, action: PayloadAction<PanelInstanceId>) {
      ...
      if (panel.minimized && state.activeLayout.terminalOwnerPanelId === panel.id) state.activeLayout.terminalOwnerPanelId = visibleTerminalReplacement(state.activeLayout);
      markDirty(state);
    },
    focusPanel(state, action: PayloadAction<PanelInstanceId>) {
      ...
      if (state.activeLayout.floatingPanelIds.includes(panel.id)) {
        state.activeLayout.floatingPanelIds = state.activeLayout.floatingPanelIds.filter((id) => id !== panel.id);
    ...
    revealPanelKind(state, action: PayloadAction<PanelKind>) {
      const panel = Object.values(state.activeLayout.instances).find((candidate) => candidate.kind === action.payload);
      if (panel) {
        panel.minimized = false;
        state.activeLayout.focusedPanelId = panel.id;
    ...
    setColumnCount(state, action: PayloadAction<number>) {
      const count = Math.max(MIN_PANEL_COLUMNS, Math.min(MAX_PANEL_COLUMNS, Math.trunc(action.payload)));
      const document = state.activeLayout;
      while (document.columns.length < count) document.columns.push({ id: `column-${document.nextColumnSequence++}`, width: 1, panelIds: [] });
      ...
      document.columns.forEach((column) => { column.width = 100 / count; });
      markDirty(state);
    },
    commitColumnWidths(state, action: PayloadAction<number[]>) {
      if (action.payload.length !== state.activeLayout.columns.length) return;
      const total = action.payload.reduce((sum, value) => sum + (Number.isFinite(value) && value > 0 ? value : 0), 0);
      if (total <= 0) return;
      state.activeLayout.columns.forEach((column, index) => { column.width = (action.payload[index]! / total) * 100; });
      markDirty(state);
    },
    movePanel(state, action: PayloadAction<{ panelId: string; columnId: string; index?: number }>) {
      ...
      column.panelIds.splice(Math.min(column.panelIds.length, Math.max(0, action.payload.index ?? column.panelIds.length)), 0, panelId);
    ...
    bringFloatingPanelToFront(state, action: PayloadAction<string>) {
      ...
      state.activeLayout.floatingPanelIds = state.activeLayout.floatingPanelIds.filter((id) => id !== action.payload);
    ...
    saveView(state, action: PayloadAction<{ id?: string; name: string }>) {
      ...
      state.userViews[id] = { id, name, createdAt: state.userViews[id]?.createdAt ?? now, updatedAt: now, document: normalizePanelLayoutDocument(state.activeLayout) };
    ...
    renameView(state, action: PayloadAction<{ viewId: string; name: string }>) {
      ...
      const name = action.payload.name.trim().slice(0, 60);
      if (view && name) { view.name = name; view.updatedAt = new Date().toISOString(); }
    },
