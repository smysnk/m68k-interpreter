...
export function encodeIndexedExtension(extension: IndexedExtension): readonly number[] {
  ...
  const indirectSelection =
    extension.indirect === 'none' ? 0 : (extension.indirect === 'postindexed' ? 4 : 0) | outer.code;
...
export function decodeIndexedExtension(words: readonly number[]): {
  ...
  if (selection !== 0)
    [outerDisplacement, cursor] = decodeDisplacement(words, cursor, selection & 3);
  return {
    extension: {
      format: 'full',
