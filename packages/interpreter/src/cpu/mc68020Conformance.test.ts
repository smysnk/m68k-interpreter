...
function coreFor(bytes: Uint8Array, options: ConstructorParameters<typeof StrictM68000Core>[0] = {}): StrictM68000Core {
...
describe('MC68020 instruction forms', () => {
  ...
  it('makes the instruction cache and CACR invalidation functionally observable', () => {
    ...
    core.state.pc = 0x1000;
    expect(core.step()).toMatchObject({ kind: 'exception', fault: { code: 'illegal-instruction' } });
  });
});
