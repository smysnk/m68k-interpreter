...
export function encodeExceptionFrame(input: ExceptionFrameInput): Uint8Array {
  ...
  const format = fault ? (input.cpuModel === 'm68010' ? 8 : input.cpuModel === 'm68020' ? 0xa : 0) : 0;
  ...
  if (format === 8) {
    const specialStatus =
      (input.write ? 0 : 0x0100) |
      (input.instruction ? 0x2000 : 0) |
      (input.functionCode ?? 0);
    set16(bytes, 8, specialStatus);
    set32(bytes, 10, input.faultAddress ?? 0);
  } else if (format === 0xa) {
    const specialStatus =
      (input.write ? 0x0040 : 0) |
      (input.instruction ? 0x0100 : 0) |
      (input.functionCode ?? 0);
...
export function decodeExceptionFrame(
  ...
  const size = format === 0 ? 8 : format === 8 && cpuModel === 'm68010' ? 58 : format === 0xa && cpuModel === 'm68020' ? 32 : 0;
  if (size === 0) throw new RangeError(`Unsupported ${cpuModel} exception-frame format ${format.toString(16)}`);
  ...
  return {
    ...
    programCounter,
    size,
    faultAddress: format === 8 ? bus.read32(address + 10) : format === 0xa ? bus.read32(address + 16) : undefined,
  };
}
