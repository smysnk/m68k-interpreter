...
export function encodeSourceInstruction(
  ...
  if (op === 'stop' || op === 'rtd' || op === 'link') {
    if (op === 'link' && size === CODE_LONG) {
      ...
      return wordsToBytes([0x4808 | addressRegister(operands[0]), immediate >>> 16, immediate]);
    }
    const opcode =
  ...
  if (op === 'pack' || op === 'unpk') {
    ...
    return wordsToBytes([
      (op === 'pack' ? 0x8140 : 0x8180) | (destination << 9) | (memory ? 0x0008 : 0) | source,
      operands[2].value,
    ]);
  }
  ...
  if (op === 'cpbcc') {
    ...
    return wordsToBytes([
      0xf000 | ((operands[0].value & 7) << 9) | ((long ? 3 : 2) << 6) | (operands[1].value & 0x3f),
      ...(long ? [(displacement >>> 16) & 0xffff, displacement & 0xffff] : [displacement & 0xffff]),
    ]);
  }
  if (op === 'cpdbcc') {
  ...
  if (bitFieldOperation[op] !== undefined) {
    ...
    const destinationIndex = ['bfextu', 'bfexts', 'bfffo'].includes(op)
      ? 1
      : op === 'bfins'
        ? 0
        : -1;
    const operand = operands[operandIndex];
    if (operand.bitField === undefined)
      throw new Error(`${op.toUpperCase()} requires {offset:width}`);
    ...
    const extension =
      (destination << 12) |
      (field.offsetRegister !== undefined
        ? 0x0800 | (field.offsetRegister << 6)
        : (field.offset & 0x1f) << 6) |
  ...
  if (op === 'cas2') {
    ...
    const extension1 = (addresses[0] << 12) | ((update[0] - 8) << 6) | (compare[0] - 8);
    const extension2 = (addresses[1] << 12) | ((update[1] - 8) << 6) | (compare[1] - 8);
    return wordsToBytes([size === CODE_WORD ? 0x0cfc : 0x0efc, extension1, extension2]);
  }
  ...
  if (CONDITION_CODE[op] !== undefined) {
    ...
    if (size === CODE_LONG) {
      return wordsToBytes([0x60ff | (CONDITION_CODE[op] << 8), displacement >>> 16, displacement]);
    }
